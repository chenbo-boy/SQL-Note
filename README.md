...
#### 查看指定数据库中的表 
    其实所有表和数据库的信息都在information_schema表里。看一下这个表信息。
...
select * from suser su INNER JOIN course co ON su.id = co.s_id;
...
select * from suser su LEFT JOIN course co ON su.id = co.s_id;
...
ALTER TABLE score ADD num FLOAT(4,1);
插入分数：
...
查看表状态
mysql> SHOW TABLE STATUS LIKE 'custom' \G;
*************************** 1. row ***************************
           Name: custom
         Engine: InnoDB
        Version: 10
